...
namespace SQLRecon.Modules
{
    internal abstract class Clr
    {
        ...
        internal static void ImpersonateAndLinked(SqlConnection con, string dll, string function, string impersonate, string linkedSqlServer, string sqlServer, string[] linkedSqlServerChain = null)
        {
            // Get the SHA-512 hash for the DLL and convert the DLL to bytes
            string[] dllArr = _convertDLLToSQLBytes(dll);
            string dllHash = dllArr[0];
            ...
            // First check to see if rpc is enabled.
            if (Config.ModuleStatus(con, "rpc", impersonate, linkedSqlServer) == false)
            {
                Print.Error(
                    $"You need to enable RPC for {linkedSqlServer} on {sqlServer} (enablerpc /rhost:{linkedSqlServer}).", true);
                // Go no further.
            ...
            // Then check to see if clr integration is enabled.
            if (Config.ImpersonateLinkedModuleStatus(con, "clr enabled", linkedSqlServer, impersonate, linkedSqlServerChain) == false)
            {
                Print.Error("You need to enable CLR (enableclr).", true);
                // Go no further.
                return;
